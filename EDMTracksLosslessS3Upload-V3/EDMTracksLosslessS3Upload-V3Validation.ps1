...
#Check number of hyphens in each file in local folder
Write-Output " "
Write-Output " "
Write-Output "Checking number of hyphens in each file in local folder."

ForEach ($LocalSourceObjectFileName In $LocalSourceObjectFileNames) {

    #Count number of hyphens in each file in local folder
    $LocalSourceObjectFileNameHyphenCount = ($LocalSourceObjectFileName.Split('-')).count-1
    
    #If local file contains more than 2 hyphens, output this and stop the script.
    If ($LocalSourceObjectFileNameHyphenCount -gt 2) {
        Write-Output "$LocalSourceObjectFileName contains $LocalSourceObjectFileNameHyphenCount hyphens.  Please review.  Exiting."
        Start-Sleep -Seconds 10
        Stop-Transcript
        Exit
    }
}
Write-Output "Hyphen count check complete."
...
ForEach ($LocalSourceObjectFileName In $LocalSourceObjectFileNames) {
    ...
    #Increase $UploadCountProgress
    $UploadCountProgress = $UploadCountProgress + 1
    ...
    Write-Output " "
    Write-Output "Starting S3 Upload $UploadCountProgress of $LocalSourceCount - $LocalSourceObjectFileName"
